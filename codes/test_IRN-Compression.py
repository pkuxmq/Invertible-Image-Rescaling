...
for test_loader in test_loaders:
    ...
    for data in test_loader:
        ...
        rlr_img = util.tensor2img(visuals['RLR'])  # uint8
        ...
        if crop_border == 0:
            ...
            cropped_srgt_img = srgt_img
        else:
            cropped_sr_img = sr_img[crop_border:-crop_border, crop_border:-crop_border, :]
            cropped_srgt_img = srgt_img[crop_border:-crop_border, crop_border:-crop_border, :]

        psnr = util.calculate_psnr(cropped_sr_img * 255, cropped_srgt_img * 255)
        ssim = util.calculate_ssim(cropped_sr_img * 255, cropped_srgt_img * 255)
        ...
        psnr_restore = util.calculate_psnr(lr_img * 255, rlr_img)
        test_results['psnr_restore'].append(psnr_restore)

        if srgt_img.shape[2] == 3:  # RGB image
            sr_img_y = bgr2ycbcr(sr_img, only_y=True)
            srgt_img_y = bgr2ycbcr(srgt_img, only_y=True)
            if crop_border == 0:
                cropped_sr_img_y = sr_img_y
                cropped_srgt_img_y = srgt_img_y
            else:
                cropped_sr_img_y = sr_img_y[crop_border:-crop_border, crop_border:-crop_border]
                cropped_srgt_img_y = srgt_img_y[crop_border:-crop_border, crop_border:-crop_border]
            psnr_y = util.calculate_psnr(cropped_sr_img_y * 255, cropped_srgt_img_y * 255)
            ssim_y = util.calculate_ssim(cropped_sr_img_y * 255, cropped_srgt_img_y * 255)
            ...
            logger.info(
                    '{:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}. LR PSNR: {:.6f} dB; SSIM: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}. PSNR Restore: {:.6f}, bpp: {:.6f}.'.
                format(img_name, psnr, ssim, psnr_y, ssim_y, psnr_lr, ssim_lr, psnr_y_lr, ssim_y_lr, psnr_restore, bpp))
        else:
            logger.info('{:20s} - PSNR: {:.6f} dB; SSIM: {:.6f}. LR PSNR: {:.6f} dB; SSIM: {:.6f}. PSNR Restore: {:.6f}, bpp: {:.6f}.'.format(img_name, psnr, ssim, psnr_lr, ssim_lr, psnr_restore, bpp))
    ...
    ave_psnr_restore = sum(test_results['psnr_restore']) / len(test_results['psnr_restore'])
    ...
    logger.info(
            '----Average PSNR Restore and bpp----\n\tpsnr_restore: {:.6f} db; bpp: {:.6f}.\n'.format(
                ave_psnr_restore, ave_bpp))
